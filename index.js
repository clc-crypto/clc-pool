...
function cutTo13Decimals(num) {
    const str = num.toString();
    const index = str.indexOf('.');
    if (index === -1)
        return str; // no decimal, return as is
...
const poolDiff = "0000FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF";
const fee = 0.02;
const feeCoinId = 0;
app.use((0, cors_1.default)()); // Cors all origins
...
let contributors = {};
let rewards = {};
if (fs_1.default.existsSync("./rewards.json"))
    rewards = JSON.parse(fs_1.default.readFileSync("./rewards.json", "utf-8"));
let mined = {};
if (fs_1.default.existsSync("./mined.json"))
    mined = JSON.parse(fs_1.default.readFileSync("./mined.json", "utf-8"));
let usedHashes = [];
function save() {
    fs_1.default.writeFileSync("./rewards.json", JSON.stringify(rewards, null, 4));
    fs_1.default.writeFileSync("./mined.json", JSON.stringify(mined, null, 4));
}
async function splitRewards(holder, miningSignature, minedHash, coinPrivate) {
    ...
    if (id === null || id === undefined)
        throw new Error("Server did not properly respond to challenge-solved: " + JSON.stringify(subRes));
    ...
    if (feeRes.message !== "success")
        throw new Error("Error paying dev fees " + JSON.stringify(feeRes));
...
app.get("/payouts/:poolsecret", async (req, res) => {
    try {
        ...
        for (const cid in rewards) {
            if (rewards[cid][poolSecret])
                result.push(Number(cid));
        }
        res.json(result);
    }
    catch (e) {
...
async function splitOffReward(id, poolSecret) {
    ...
    if (splitRes.message !== "success")
        throw new Error("Error splitting coin #" + id + ", " + splitRes.error);
    return splitId;
}
async function splitOffAndMergeReward(id, targetId, poolSecret) {
    const key = ecdsa.keyFromPrivate(mined[id], "hex");
    const minedCoin = (await (await fetch(server + "/coin/" + id)).json()).coin;
    // Get ledger length
    ...
    if (mergeRes.message !== "success")
        throw new Error("Error merging coin " + JSON.stringify(mergeRes));
}
app.get("/payout/:poolsecret", async (req, res) => {
    try {
        ...
        if (!addr)
            throw new Error("addr query param not provided");
        let rewardId = -1;
        let key = null;
        for (const minedId in mined) {
            if (!rewards[minedId][poolSecret])
                continue;
            if (rewardId !== -1) {
        ...
        if (rewardId === -1 || key === null)
            throw new Error("Already paid out your rewards, or you have not yet mined any.");
        ...
        if (transRes.message !== "success")
            throw new Error("Error transacting coin #" + rewardId + ", " + transRes.error);
        save();
...
app.get("/challenge-solved", async (req, res) => {
    try {
        ...
        const miningSignature = req.query.sign;
        const minedHash = req.query.hash;
        const poolSecret = req.query.poolsecret;
        const coinPrivate = req.query.key;
        const netJob = await (await fetch(server + "/get-challenge")).json();
        const key = ecdsa.keyFromPublic(holder, 'hex');
        if (ecdsa.keyFromPrivate(coinPrivate, "hex").getPublic().encode("hex", false) !== req.query.holder)
            throw new Error("Invlaid secret key");
        if (!key.verify(sha256(holder), miningSignature) && miningSignature !== "split")
            throw new Error("Invalid mining signature.");
        if (BigInt("0x" + poolDiff) < BigInt("0x" + minedHash))
            throw new Error("Mined hash does not meet difficulty criteria.");
        if (sha256(key.getPublic().encode("hex", false) + netJob.seed) !== minedHash)
            throw new Error("Invalid mined hash.");
        if (usedHashes.includes(minedHash))
            throw new Error("This hash has already been submitted!");
        console.log("0x" + netJob.diff + " | 0x" + minedHash);
        if (!contributors[poolSecret])
            contributors[poolSecret] = 0;
